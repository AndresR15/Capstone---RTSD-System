...
class audio_only_model(Model):
    def __init__(self, optimizer="Adam", training=None):
        super(audio_only_model, self).__init__()

        # the first branch operates on the audio input
        self.model_audio = Sequential(name = "model_audio")
        ...
        # combined outputs
        self.dense = Dense(4, activation="softmax", input_shape=(None, 512), name="output")
...
#
# model = keras.models.load_model('F:\Capstone Project\Capstone---RTSD-System\Code\Misc\Saved_data\Trained_models\Weights\Audio_only/model')
#
# print(model.summary())